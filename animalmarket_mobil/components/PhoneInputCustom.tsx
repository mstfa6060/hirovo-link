...
import { getCountry } from 'react-native-localize'; // Native replacement for expo-localization
...
export default function PhoneInputCustom({ value, onChange }: Props) {
    ...
    useEffect(() => {
        const getDefaultCountry = () => {
            // Use react-native-localize to get country code
            const localeCountry = getCountry()?.toUpperCase();
            if (localeCountry && isCountryCode(localeCountry)) {
                setCountryCode(localeCountry);
            } else {
                setCountryCode('TR'); // Default to Turkey
            }
        };
